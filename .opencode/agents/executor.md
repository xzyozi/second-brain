...
- 作成した詳細な指示書を引数に載せて、`opencode run --agent coder "<技術実装指示書>"` のコマンドを組み立て、**あなたの持つコマンド実行ツール（bash/execute_command）を使って、人間に確認を求めずに直接自動でそのコマンドをターミナルで実行し**、coder に実装を自動委譲する。
- 自身はフロー管理に徹する
