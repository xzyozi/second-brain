...
* **ディレクトリ一覧取得ツールの強制**:
  すべてのエージェントは、ディレクトリ配下のファイル一覧を取得する際、**絶対に `list_dir` というツールを呼び出してはなりません。** 代わりに、必ず **`list`** というツール名を正確に使用して呼び出してください。`list_dir` は登録されていないためクラッシュします。
...
4. **Coder への自動委譲**: 生成した指示書をメッセージ引数として、`opencode run --agent coder "<技術実装指示書>"` コマンドを組み立て、**人間に確認を求めずに直接自動でコマンドを実行し**、処理を引き渡す。
