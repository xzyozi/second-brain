...
| :------- | :--------------------------------------------------------------- |
| 文書番号 | SBOS-BD-002                                                      |
| 版数     | Rev.4.0（実在OSSスタック統合再設計・Windows Native対応版）       |
| 改訂日   | 2026年7月28日                                                     |
| 作成日   | 2026年7月27日                                                     |
| 関連文書 | SBOS-DD-003（詳細設計書 Rev.4.0）、SBOS-MULTI-001 Rev.2.1、SBOS-OP-001 Rev.3.3、SBOS-OSS-001/002 |
...
特に **Rev.4.0 改版** においては、従来の自前手書きループ（`orchestrator.py` や `agent_client.py` 等）を廃止し、**LangGraph（状態管理・条件分岐グラフ）、LiteLLM（統一LLMインターフェース）、Aider（コード自動編集・Gitワーキングツリー管理）、Ruff（一次高速静的解析）、Reviewdog（差分行アノテーション表示）** という実在OSSスタックへ全面的に置き換えたアーキテクチャを規定する。

### 1.2 構造的課題とOSS統合による解決

| 従来自前実装の課題 | OSS統合（Rev.4.0）での解決策 | 担当コンポーネント |
| :--- | :--- | :--- |
...
| 自作 AST マージの破綻・コード一部消失 | 堅牢な Git ワーキングツリー差分編集と修復 | **Aider** (`aider-chat` `--no-auto-commit`) |
...
| 3 | Coder Agent + `_merge_python_code` | **Aider** (`aider-chat`) | Apache-2.0 | `--no-auto-commit` で衛星ワーキングツリーに変更を反映。自動コミットは行わない。 |
...
[ tools/orchestrator_graph.py ]  ← LangGraph StateGraph (母艦)
  ...
  plan_node (Executor役: LiteLLM 経由で要件指示書生成)
         │
         ▼
  code_node (Aider を衛星リポジトリに対して実行: --no-auto-commit)
         │
         ▼
  lint_node (Ruff 高速静的解析) ─────[失敗]───┐
         │ [PASSED]                           │ (指摘を message に追加)
         ▼                                    │
  test_node (pytest + json-report) ──[失敗]───┤
         │ [PASSED]                           │
         ▼                                    │
  review_node (LiteLLM レビュー ＋ Reviewdog 出力)
         ...
         └─ [changes_requested] ──────────────┤
                                              ▼
                                    (round < max_round ?) ──[Yes]──► code_node
                                              │ [No: 上限到達]
                                              ▼
                                       escalate_node (tasks.md round:3 更新 / B7 ブロッカー化)
```
...
2. **Aider のコミット制御**:
   Aider 起動パラメータに `--no-auto-commit` を指定。Aider はファイルの修正のみを行い、`git commit` は行わない。
3. **人間の最終承認**:
