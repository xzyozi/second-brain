...
| :------- | :--------------------------------------------------------------- |
| 文書番号 | SBOS-BD-002                                                      |
| 版数     | Rev.4.1（バグ修正・Windows Native詳細化・リトライ安全回路追加版）|
| 改訂日   | 2026年7月28日                                                     |
| 作成日   | 2026年7月28日                                                     |
| 関連文書 | SBOS-DD-003（詳細設計書 Rev.4.1）、SBOS-MULTI-001 Rev.2.1、SBOS-OP-001 Rev.3.3（※旧自前実装向け。新アーキテクチャ移行に伴い大半が改訂対象）、SBOS-OSS-001/002 |
...
特に **Rev.4.1 改版** においては、従来の自前手書きループ（`orchestrator.py` や `agent_client.py` 等）を廃止し、**LangGraph（状態管理・条件分岐グラフ）、LiteLLM（統一LLMインターフェース）、Aider（コード自動編集・Gitワーキングツリー管理）、Ruff（一次高速静的解析）、Reviewdog（差分行アノテーション表示）** という実在OSSスタックへ全面的に置き換えたアーキテクチャを規定する。

### 1.2 構造的課題とOSS統合による解決

| 従来自前実装の課題 | OSS統合（Rev.4.1）での解決策 | 担当コンポーネント |
| :--- | :--- | :--- |
