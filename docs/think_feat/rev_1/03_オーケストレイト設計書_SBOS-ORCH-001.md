...
コード内のいかなる変更も人間による `permission: ask` コミット承認を待つ必要があるため、レビュアーが Git コミット間の diff を参照することはできない。
`orchestrator.py` のファイル書き込みインターフェース `_write_files()` で以下のロジックを実行し、メモリ内で差分を生成してレビューAgentに渡す。
